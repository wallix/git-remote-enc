...
#
# This can change files even when Rust and the base image are current: `nix flake
# update` resolves both input branches again and rewrites flake.lock when either
# moves. Review the diff, then run ./build.sh --verify.
#
